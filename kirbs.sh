...
case "$choice" in
    ...
    5)
        ...
        ;;
    6)
        if command -v xdg-open &>/dev/null; then xdg-open "$DIR/templates/manual.html"; elif command -v open &>/dev/null; then open "$DIR/templates/manual.html"; else sed "s/<[^>]*>/ /g" "$DIR/templates/manual.html" | less; fi
        ;;
    q|Q)
