...
PLAYLIST = [
    # 🌹 Lana Del Rey
    {"title": "Lana Del Rey — Summertime Sadness",           "artist": "Lana Del Rey", "yt": "TdrL3QxjyVw"},
    {"title": "Lana Del Rey — Young and Beautiful",          "artist": "Lana Del Rey", "yt": "domMDtbODaY"},
    {"title": "Lana Del Rey — Video Games",                  "artist": "Lana Del Rey", "yt": "J4-6VYiTBE0"},
    {"title": "Lana Del Rey — Born To Die",                  "artist": "Lana Del Rey", "yt": "Bag1gUxuU0g"},
    {"title": "Lana Del Rey — Blue Jeans",                   "artist": "Lana Del Rey", "yt": "9YGiAHuZCNc"},
    {"title": "Lana Del Rey — Ride",                         "artist": "Lana Del Rey", "yt": "oUCmmN3dPZg"},

    # 🕺 Bee Gees
    {"title": "Bee Gees — Stayin' Alive",                    "artist": "Bee Gees",     "yt": "I_izvAbhExY"},
    {"title": "Bee Gees — Night Fever",                      "artist": "Bee Gees",     "yt": "hA5VH2vJBls"},
    {"title": "Bee Gees — How Deep Is Your Love",            "artist": "Bee Gees",     "yt": "XpqqjU7u5Yc"},
    {"title": "Bee Gees — More Than a Woman",                "artist": "Bee Gees",     "yt": "UgKMCJnzoS8"},

    # 🎤 Michael Jackson
    {"title": "Michael Jackson — Thriller",                  "artist": "Michael Jackson", "yt": "sOnqjkJTMaA"},
    {"title": "Michael Jackson — Billie Jean",               "artist": "Michael Jackson", "yt": "Zi_XLOBDo_Y"},
    {"title": "Michael Jackson — Beat It",                   "artist": "Michael Jackson", "yt": "oRdxUFDoQe0"},
    {"title": "Michael Jackson — Man in the Mirror",         "artist": "Michael Jackson", "yt": "PivWY9wn5ps"},
    {"title": "Michael Jackson — Speed Demon",               "artist": "Michael Jackson", "yt": "1CFP9bPnN_I"},

    # 🎹 Chopin
    {"title": "Chopin — Nocturne Op. 9 No. 2",              "artist": "Chopin",        "yt": "9E6b3swbnWg"},
    {"title": "Chopin — Ballade No. 1 in G minor",          "artist": "Chopin",        "yt": "VmFmAvwAOXI"},
    {"title": "Chopin — Fantasie Impromptu",                 "artist": "Chopin",        "yt": "tvm2ZsRv3Zs"},
    {"title": "Chopin — Raindrop Prelude Op. 28 No. 15",    "artist": "Chopin",        "yt": "XeX4X_1_lo0"},
    {"title": "Chopin — Waltz in C-sharp minor",            "artist": "Chopin",        "yt": "X1mECSPKvbE"},

    # 🖤 Billie Eilish
    {"title": "Billie Eilish — bad guy",                     "artist": "Billie Eilish", "yt": "DyDfgMOUjCI"},
    {"title": "Billie Eilish — Ocean Eyes",                  "artist": "Billie Eilish", "yt": "viimfQi_pUw"},
    {"title": "Billie Eilish — You Should See Me in a Crown","artist": "Billie Eilish", "yt": "ao_XfSMDYME"},
    {"title": "Billie Eilish — lovely (with Khalid)",        "artist": "Billie Eilish", "yt": "akjqkZAFgcA"},
    {"title": "Billie Eilish — Happier Than Ever",           "artist": "Billie Eilish", "yt": "5GJWxDKyk3A"},

    # ✨ CAS (Cavetown alias / artist)
    {"title": "CAS — I Am a Dreamer",                        "artist": "CAS",           "yt": "VuNIsY6JdUw"},
    {"title": "CAS — Thinking of You",                       "artist": "CAS",           "yt": "dVXMKtYyGnI"},
]
...
class CosmicPlayer:
    ...
    def show_playlist(self, filter_artist=None):
        artists = {}
        for i, t in enumerate(PLAYLIST):
            a = t['artist']
            if filter_artist and filter_artist.lower() not in a.lower():
                continue
            artists.setdefault(a, []).append((i, t))

        ARTIST_COLORS = {
            "Lana Del Rey":    '\033[38;5;218m',   # pink
            "Bee Gees":        '\033[93m',           # yellow
            "Michael Jackson": '\033[96m',           # cyan
            "Chopin":          '\033[97m',           # white
            "Billie Eilish":   '\033[90m',           # dark
            "CAS":             '\033[95m',           # magenta
        }
        ARTIST_ICONS = {
            "Lana Del Rey":    "🌹",
        ...
        for artist, tracks in artists.items():
            col  = ARTIST_COLORS.get(artist, C['cyan'])
            icon = ARTIST_ICONS.get(artist, "♪")
            print(f"\n  {col}{C['bold']}{icon}  {artist}{C['reset']}")
            for i, t in tracks:
                marker = f"{C['pink']}▶ {C['reset']}" if i == self.track_idx else "  "
                print(f"    {marker}[{i+1:02d}] {t['title'].split(' — ',1)[-1]}")
        print()
...
def main():
    ...
    print(f"  {C['green']}Commands: [P] Play  [N] Next  [B] Back  [R] Random  [S] Stop")
    print(f"  [L] Playlist  [H] History  [Q] Quit")
    print(f"  Artist filter: lana · beegees · mj · chopin · billie · cas{C['reset']}\n")
    ...
    while True:
        ...
        # ── Artist shortcuts ──────────────────────────────
            ...
            player.show_playlist('Lana Del Rey')
        elif cmd in ('beegees', 'bee gees', 'bee'):
            player.show_playlist('Bee Gees')
        elif cmd in ('mj', 'michael', 'jackson'):
            player.show_playlist('Michael Jackson')
        elif cmd in ('chopin',):
            player.show_playlist('Chopin')
        elif cmd in ('billie', 'eilish'):
            player.show_playlist('Billie Eilish')
        elif cmd in ('cas',):
            player.show_playlist('CAS')
        # ── Play by number ────────────────────────────────
        elif cmd.isdigit():
            idx = int(cmd) - 1
            if 0 <= idx < len(PLAYLIST):
            ...
            print(f"  {C['void']}Unknown command. Try P, N, B, R, L, H, S, Q, a number, or an artist name.{C['reset']}")
